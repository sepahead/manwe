...
//!
//! This module implements the YOLOv8 object detection architecture for inference
//! using the Candle deep learning framework with Metal GPU acceleration.
//!
//! # Architecture
//! - DarkNet backbone for feature extraction
//! - Feature Pyramid Network (FPN) neck for multi-scale features
//! - Detection head for bounding box and class predictions
//!
//! # Precision
//! All computations use FP32 (single precision) for consistency across benchmarks.
...
///
/// These control the depth (number of layers), width (channel counts),
/// and ratio (expansion factor) of the network.
...
impl Multiples {
    ...
    /// YOLOv8s (small) configuration - used for benchmarking.
    /// ~11M parameters, good balance of speed and accuracy.
    pub fn s() -> Self {
        Self {
            depth: 0.33,
            width: 0.50,
    ...
    /// Calculate filter sizes for the detection head.
...
/// Cross Stage Partial (CSP) bottleneck with 2 convolutions (C2f).
#[derive(Debug)]
struct C2f {
    cv1: ConvBlock,
    cv2: ConvBlock,
...
/// Distribution Focal Loss layer for bounding box regression.
#[derive(Debug)]
struct Dfl {
    conv: Conv2d,
    num_classes: usize,
...
/// Detection head output containing predictions and anchor information.
...
/// Generate anchor points for detection at multiple scales.
fn make_anchors(
    xs0: &Tensor,
    xs1: &Tensor,
    xs2: &Tensor,
...
///
/// This is the complete YOLOv8 architecture for object detection,
/// consisting of a DarkNet backbone, FPN neck, and detection head.
///
/// # Precision
/// Uses FP32 precision for all computations.
#[derive(Debug)]
pub struct YoloV8 {
    net: DarkNet,
    fpn: YoloV8Neck,
...
impl YoloV8 {
    /// Load a YOLOv8 model from safetensors weights.
    ///
    /// # Arguments
    /// * `vb` - Variable builder with loaded weights
    /// * `m` - Model size multipliers (use `Multiples::s()` for YOLOv8s)
...
impl Module for YoloV8 {
    ...
    ///
    /// # Arguments
    /// * `xs` - Input tensor of shape [batch, 3, height, width] in FP32
    ///
    /// # Returns
    /// Predictions tensor of shape [batch, num_predictions, 4 + num_classes]
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let _enter = self.span.enter();
